class PokemonRater {
    ...
    static ING_SPECIES = Object.keys(PokemonRater.SPECIES_TO_SPECIALTY)
        .filter(species => PokemonRater.SPECIES_TO_SPECIALTY[species] === 'Ingredient');

    static XP_HARD_SPECIES = [
    ...
    matchSpecialty() {
        this.specialty = PokemonRater.SPECIES_TO_SPECIALTY[this.species];
    ...
    rateNature() {
        ...
        if (PokemonRater.XP_HARD_SPECIES.includes(this.species)) {
    ...
    rateIngredients() {
        if (PokemonRater.ING_SPECIES.includes(this.species)) {
            const ingredientKey = [this.species, this.ing30, this.ing60].join('|');
            this.ingredientRating = PokemonRater.ING_COMBI_RATINGS[ingredientKey];
        } else {
    ...
    rateTierlist() {
        this.tierlistRating = PokemonRater.SPECIES_TO_TIERLIST[this.species];
    }
